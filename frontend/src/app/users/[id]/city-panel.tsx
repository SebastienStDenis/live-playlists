...
export function CityPanel({
  ...
  if (city !== null && !editing) {
    return (
      <CityCard userId={userId} city={city} onEdit={() => setEditing(true)} />
    );
  }
  return (
    <CitySearch
...
function CityCard({
  ...
  const [state, clearAction, pending] = useActionState(
    clearCity.bind(null, userId),
    { error: null },
  );
...
function CitySearch({
  ...
  function select(city: City) {
    startTransition(async () => {
      const result = await setCity(userId, city.geonameid);
      if (result.error) {
