...
Needs SPOTIFY_CLIENT_ID/SECRET/REFRESH_TOKEN in .env (run cli.spotify_auth
...
Usage (from backend/): uv run python -m cli.spotify_verify
"""

import asyncio

from app.clients.spotify import SpotifyApiError, SpotifyClient, track_uri
...
async def main() -> None:
    ...
    if not settings.spotify_refresh_token:
        raise SystemExit("Set SPOTIFY_REFRESH_TOKEN in .env first (run cli.spotify_auth).")
...
async def run_checks(client: SpotifyClient) -> None:
    ...
    playlist = await client.create_playlist(
        ...
        "Throwaway playlist created by cli.spotify_verify.",
    )
    ...
    try:
        ...
        await client.update_playlist_details(
            playlist.id, "next-fm verification (renamed)", "Renamed by cli.spotify_verify."
        )
